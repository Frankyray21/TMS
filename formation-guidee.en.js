...
(function () {
  ...
  function renderSommaire() {
    ...
    var attDesc = allDone ? "Enter your name: your personalized certificate is ready to print or save as a PDF." : "Go through the topics and pass the quiz in each module (80 %) to generate your certificate.";
  ...
  function renderZones() {
    ...
    return '<div class="fg-kb"><p class="lead">Underground, the most exposed areas range from the neck to the ankles. <strong style="color:#e2e8f0">Rotate the 3D model</strong> and display the structures (muscles, bones, joints, nerves):</p>'
      ...
      + '<p class="corps-3d-note">Anatomical model · rotate the body, display the structures and hover over a marker. <span>Model: Z-Anatomy (CC BY-SA).</span></p></div></div>';
  }
  ...
  function renderFacteurs() {
    ...
    return '<div class="fg-kb"><p class="lead">In workers, the risk comes mainly from the <strong style="color:#e2e8f0">task</strong>, the <strong style="color:#e2e8f0">body</strong> and the <strong style="color:#e2e8f0">immediate environment</strong>. We distinguish three groups, which add up:</p>'
      + '<div style="display:grid;grid-template-columns:minmax(180px,.85fr) repeat(3,1fr);gap:16px;align-items:stretch;margin:1rem 0 1.1rem">'
  ...
  function renderQuiz(m) {
    ...
    if (sc.passed) { resultText = '✓ Module passed (' + sc.score + ' / ' + sc.total + ') — you can continue.'; resultColor = '#34d399'; }
    else if (sc.answered === sc.total) { resultText = 'Not yet: ' + sc.score + ' / ' + sc.total + ' (you need ' + sc.need + '). Review the explanations and try again.'; resultColor = '#f87171'; }
    else { resultText = 'Answer all the questions (' + sc.answered + ' / ' + sc.total + ').'; resultColor = '#8694ad'; }

    return '<p class="lead">Answer the ' + sc.total + ' questions in the <b style="color:#fff">' + esc(m.title) + '</b> module. <span style="color:#9aa7bd">The correction and explanation appear as soon as you choose an answer</span>; score at least 80 % to pass the module.</p>'
