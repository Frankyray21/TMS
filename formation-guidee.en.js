...
(function () {
  ...
  function renderSommaire() {
    ...
    return '<div><section style="position:relative;overflow:hidden;border-bottom:1px solid #1e293b;background:radial-gradient(120% 100% at 80% -10%,#16202f 0%,#0a0e17 55%)">'
      ...
      + '<div style="display:inline-flex;align-items:center;gap:10px;font-family:\'Barlow Condensed\',sans-serif;font-weight:800;letter-spacing:.14em;text-transform:uppercase;font-size:1rem;color:#ef5a5c"><span style="width:7px;height:7px;border-radius:50%;background:#d22325;box-shadow:0 0 8px rgba(210,35,37,.9)"></span>Guided training · New workers</div>'
      + '<div id="fg-total-time" title="Time already spent in the training" style="display:inline-flex;align-items:center;gap:7px;font-family:\'Barlow Condensed\',sans-serif;font-weight:800;font-size:.92rem;color:#e2e8f0;white-space:nowrap;font-variant-numeric:tabular-nums;background:rgba(30,41,59,.75);border:1px solid #2b3a52;border-radius:999px;padding:8px 14px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef5a5c" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><path d="M12 7.5V12l3 1.8"></path></svg><span style="color:#8694ad;text-transform:uppercase;letter-spacing:.08em;font-size:.76rem">Time spent</span>' + fmtDur(liveTotalMs()) + '</div>'
      + '</div>'
      + '<h1 class="fg-h1" style="font-family:\'Barlow Condensed\',sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:.01em;line-height:1.02;font-size:clamp(2.3rem,5vw,3.5rem);margin:0 0 16px">MSD prevention,<br><span class="mr-plate" style="color:#ef5a5c">step by step.</span></h1>'
  ...
  /* ---------- app appreciation survey (before saving) ---------- */
  ...
  function starRow() {
    ...
    for (var i = 1; i <= 5; i++) { var on = i <= r; s += '<button type="button" class="fg-star' + (on ? ' on' : '') + '" data-star="' + i + '" aria-label="' + i + ' out of 5" aria-pressed="' + on + '">' + STAR_SVG + '</button>'; }
    return s;
  }
  ...
  function appSignBlock() {
    return '<div class="att-sign" style="margin:0 0 20px">'
      ...
      + '<span id="attSigPh" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:.95rem;font-style:italic;pointer-events:none">✍ Sign here</span>'
      + '</div>'
      + '<div style="display:flex;justify-content:flex-end;margin-top:8px"><button type="button" id="attSigClear" style="font-family:\'Barlow Condensed\',sans-serif;font-weight:700;text-transform:uppercase;letter-spacing:.03em;font-size:.78rem;color:#8694ad;background:transparent;border:1px solid #1e293b;border-radius:999px;padding:7px 16px;cursor:pointer">↺ Clear</button></div>'
      + '</div>';
  }
  ...
  function renderViewer() {
    ...
    return '<div><div style="position:sticky;top:var(--fg-header-h,91px);z-index:30;background:rgba(13,19,32,.94);-webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px);border-bottom:1px solid #1e293b">'
      ...
      + '<div id="fg-timer" title="Time spent in the training" style="display:inline-flex;align-items:center;gap:6px;font-family:\'Barlow Condensed\',sans-serif;font-weight:800;font-size:.9rem;color:#e2e8f0;white-space:nowrap;font-variant-numeric:tabular-nums;background:rgba(30,41,59,.75);border:1px solid #2b3a52;border-radius:999px;padding:5px 12px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef5a5c" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><path d="M12 7.5V12l3 1.8"></path></svg><span data-timer>' + fmtClock(liveTotalMs()) + '</span></div>'
  ...
  function renderCustom(n) {
    ...
    if (n.custom === 'cTypes') return '<div class="fg-kb"><p class="lead">The most common forms of MSD in workers. <strong style="color:var(--accent-l)">The lower back is by far the most affected area.</strong></p><div class="icards tms-c">'
      + '<article class="imgcard fg-typecard" data-type="lombalgie" role="button" tabindex="0" aria-label="View details: Low-back pain" style="cursor:pointer;border:2px solid #d22325;box-shadow:0 0 0 4px rgba(210,35,37,.16),0 14px 38px rgba(0,0,0,.45)"><div style="position:absolute;top:.65rem;left:.65rem;z-index:3;background:linear-gradient(135deg,#e23a3c,#a81a1c);color:#fff;font-family:\'Barlow Condensed\',sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:.06em;font-size:.68rem;padding:.28rem .6rem;border-radius:999px;box-shadow:0 2px 8px rgba(0,0,0,.4)">★ Most common</div><div class="media" style="height:220px;background:#0a0e17"><img src="images/tms_lombalgie.jpeg" alt="Low-back pain" loading="lazy" decoding="async" style="object-fit:contain;display:block"></div><div class="body"><h4 style="color:#ef5a5c">Low-back pain</h4><p>Pain in the lower back — <strong style="color:#e2e8f0">the most affected area</strong> in workers.</p></div></article>'
      + '<article class="imgcard fg-typecard" data-type="tendinite" role="button" tabindex="0" aria-label="View details: Tendinitis" style="cursor:pointer"><div class="media" style="height:220px;background:#0a0e17"><img src="images/tms_tendinite.jpeg" alt="Tendinitis" loading="lazy" decoding="async" style="object-fit:contain;display:block"></div><div class="body"><h4>Tendinitis</h4><p>Inflammation of a tendon: shoulder, elbow, wrist.</p></div></article>'
      + '<article class="imgcard fg-typecard" data-type="bursite" role="button" tabindex="0" aria-label="View details: Bursitis" style="cursor:pointer"><div class="media" style="height:220px;background:#0a0e17"><img src="images/tms_bursite.jpeg" alt="Bursitis" loading="lazy" decoding="async" style="object-fit:contain;display:block"></div><div class="body"><h4>Bursitis</h4><p>Inflammation of the bursae: knees, shoulders.</p></div></article>'
      + '<article class="imgcard fg-typecard" data-type="carpien" role="button" tabindex="0" aria-label="View details: Carpal tunnel" style="cursor:pointer"><div class="media" style="height:220px;background:#0a0e17"><img src="images/tms_carpien.jpeg" alt="Carpal tunnel syndrome" loading="lazy" decoding="async" style="object-fit:contain;display:block"></div><div class="body"><h4>Carpal tunnel syndrome</h4><p>Compression of the median nerve at the wrist.</p></div></article></div></div>';
  ...
  function openTypeFiche(k) {
    ...
    box.innerHTML = '<div class="zfm-dialog" role="dialog" aria-modal="true" aria-label="' + esc(t.title) + '">'
      + '<button type="button" class="zf-close" aria-label="Close">✕</button>'
      + '<img src="' + t.img + '" alt="' + esc(t.title) + '" class="zf-img" loading="lazy" decoding="async">'
      + '<div class="zf-body"><h4 class="zf-title">' + esc(t.title) + '</h4>'
      + '<div class="zf-tms"><span>Type of MSD</span></div>'
      + '<p class="zf-desc">' + esc(t.desc) + '</p>'
      + '<p class="zf-h">Risk factors</p><ul class="zf-list">' + t.risk.map(function (x) { return '<li>' + esc(x) + '</li>'; }).join('') + '</ul>'
      + '<p class="zf-h">Prevention</p><ul class="zf-list">' + t.prev.map(function (x) { return '<li>' + esc(x) + '</li>'; }).join('') + '</ul>'
      + '</div></div>';
    box.hidden = false;
    var cl = box.querySelector('.zf-close');
  ...
  function renderQuiz(m) {
    ...
    var qs = m.quiz.map(function (q, qi) {
      ...
      if (answered) {
        ...
        fb += '<div class="qfb" data-qfb-key="fg:' + esc(m.id) + ':' + qi + '" data-qfb-ref="' + esc('M' + m.num + ' · Q' + (qi + 1)) + '" data-qfb-quiz="' + esc('Guided training · Module ' + m.num + ' · ' + m.title) + '" data-qfb-q="' + esc(q.q) + '"></div>';
      }
  ...
  /* ---------------- ACTIONS ---------------- */
  /* Real height of the site header -> --fg-header-h, used by the reader's sticky bar.
     Hardcoded (59px) it slid UNDER the site header on mobile, where that header is
     ~91px tall: the timer and module title were clipped there.
     The height changes without a "resize" event (session.js injects the sign-in chip,
     then the employee name after login, and font swap alters it) -> ResizeObserver. */
  ...
  /* All modules passed: take the worker straight to the attestation confirmation. */
  ...
  function certDetailDoc() {
    ...
    return '<div id="certDocDetail" style="background:#fff;color:#111;border-radius:14px;padding:30px 28px;box-shadow:0 10px 30px rgba(0,0,0,.4);width:600px;box-sizing:border-box;font-family:\'Barlow\',sans-serif">'
      ...
      + '<div style="color:#333;font-size:.86rem;margin-bottom:10px">On ' + d + '</div>'
      + (state.sigData ? '<div style="margin:0 auto 12px"><img src="' + state.sigData + '" alt="Signature" style="height:54px;object-fit:contain;display:block;margin:0 auto"><div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.12em;color:#999">Worker\'s signature</div></div>' : '')
      + '</div>'
  ...
  function initCert() {
    ...
    function upd() {
      ...
      // name changed: re-arm the "Save" button and hide the previous message
      var btn = document.getElementById('attSave');
      if (btn && btn.dataset.done) { delete btn.dataset.done; btn.textContent = '✔ Save my training'; btn.style.background = 'linear-gradient(135deg,#10b981,#0e9f6e)'; var msg = document.getElementById('saveMsg'); if (msg) msg.hidden = true; }
      syncSaveBtn();
    }
    syncSaveBtn();
  ...
  function bind() {
    ...
    // MSD type cards: clickable (detailed fiche), like the knowledge base
    ...
    // app appreciation survey (stars + comment) in the attestation
  ...
  function init() {
    ...
    if (!app) return;
    // Local Draco decoder (vendor/draco/): the Draco-compressed 3D body works
