...
(function () {
  ...
  /* Total « en direct » : temps enregistré + le segment en cours (non encore vidé).
     Se fige quand l'onglet est masqué, car _tT0 est remis à 0 par timePause(). */
  function liveTotalMs() { var t = totalTimeMs(); if (_tKey && _tT0) { var dt = _tNow() - _tT0; if (dt > 0 && dt < 21600000) t += dt; } return t; }
  /* Format horloge pour le timer affiché : MM:SS, ou H:MM:SS au-delà d'une heure. */
  ...
  function renderSommaire() {
    ...
    var resumeLabel = 'module ' + (resumeStep.mi + 1) + (resumeStep.kind === 'quiz' ? ' · quiz' : ' · notion ' + (resumeStep.ni + 1));
    var startLabel = allDone ? 'Revoir la formation' : (started ? 'Reprendre' + (resumeState.stepKey ? ' : ' + resumeLabel : '') : 'Commencer la formation');
    var progressHint = allDone ? 'Bravo, les 5 quiz sont réussis !' : (started ? 'Ta consultation est enregistrée. Les quiz valident les modules.' : 'À commencer, à ton rythme.');
    ...
    var cards = MODULES.map(function (m, mi) {
      ...
      var statusLabel = isDone ? 'Quiz validé' : (isStarted ? 'En cours' : 'À commencer');
      ...
      var items = m.notions.map(function (n, ni) {
        ...
        return notionRow((mi + 1) + '.' + (ni + 1), n.title, gi, consulted, '#0d1320', '#1e293b', consulted ? 'Consultée' : '');
      }).join('');
      var giq = st.findIndex(function (s) { return s.kind === 'quiz' && s.mi === mi; });
      items += notionRow('✓', 'Quiz du module', giq, isDone, 'rgba(210,35,37,.06)', 'rgba(210,35,37,.22)', isDone ? 'Validé' : 'À valider');
    ...
    var attTitle = allDone ? attText('Formation réussie', 'Training passed') : 'Termine les 5 modules pour débloquer';
    var attDesc = allDone ? attStatusDescription() : "Parcours les notions et réussis le quiz de chaque module pour générer ton attestation.";
    ...
    return '<div><section style="position:relative;overflow:hidden;border-bottom:1px solid #1e293b;background:radial-gradient(120% 100% at 80% -10%,#16202f 0%,#0a0e17 55%)">'
      ...
      + '<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center"><span class="fg-pct" style="font-family:\'Barlow Condensed\',sans-serif;font-weight:800;font-size:2.6rem;line-height:1;color:#fff">' + pct + '%</span><span style="font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;color:#8694ad;font-weight:700">quiz validés</span></div></div>'
      + '<div style="font-family:\'Barlow Condensed\',sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:.04em;font-size:1.05rem;color:#fff">' + doneCount + ' / ' + total + ' modules validés</div>'
      + '<div style="color:#cbd5e1;font-size:.86rem;margin-top:6px">' + visitedCount + ' / ' + totalNotions + ' notions consultées</div>'
  ...
  function appSurveyBlock() {
    return '<div class="att-survey" style="margin:0 0 20px">'
      + '<label style="display:flex;align-items:center;gap:9px;font-size:.82rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#e2e8f0;margin-bottom:8px">Ton avis sur l\'application</label>'
  ...
  function appSignBlock() {
    return '<div class="att-sign" style="margin:0 0 20px">'
      + '<label style="display:flex;align-items:center;gap:9px;font-size:.82rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#e2e8f0;margin-bottom:8px"><span style="flex:0 0 auto;width:22px;height:22px;border-radius:50%;background:#d22325;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:.82rem;font-weight:800">2</span> Signe ton attestation</label>'
  ...
  function certBlock() {
    ...
    var d = certDate.toLocaleDateString('fr-CA', { year: 'numeric', month: 'long', day: 'numeric' });
  ...
  function openZoneFiche(k) {
    ...
    box.innerHTML = '<div class="zfm-dialog" role="dialog" aria-modal="true" aria-label="' + esc(z.nom) + '">'
      ...
      + '<p class="zf-h">Bons réflexes</p><ul class="zf-list">' + z.conseils.map(function (c) { return '<li>' + esc(c) + '</li>'; }).join('') + '</ul></div></div>';
    box.hidden = false;
    clearZoneActive();
  ...
  function renderZones() {
    ...
    return '<div class="fg-kb"><p class="lead">Sous terre, les zones les plus exposées vont de la nuque aux chevilles. <strong style="color:#e2e8f0">Tourne le modèle 3D</strong> et affiche les structures (muscles, os, articulations, nerfs) :</p>'
      ...
      + '<div class="fg-zone-pick"><p class="fg-zone-pick-t">Les zones du corps les plus touchées — clique une zone pour voir les TMS fréquents et les bons réflexes. <span class="fg-zone-prog" id="fgZoneProg"></span></p><div class="fg-zone-chips">' + chips + '</div></div>'
  ...
  function renderQuiz(m) {
    ...
    var qs = m.quiz.map(function (q, qi) {
      ...
      if (answered) {
        ...
        fb += '<div class="qfb" data-qfb-key="fg:' + esc(m.id) + ':' + qi + '" data-qfb-ref="' + esc('M' + m.num + ' · Q' + (qi + 1)) + '" data-qfb-quiz="' + esc('Formation guidée · Module ' + m.num + ' · ' + m.title) + '" data-qfb-q="' + esc(q.q) + '"></div>';
      }
  ...
  var ATT_EN = false, attMemory = null, attSending = false, attRevision = 0, attCaptureCancel = null, attPadReset = null, attIdentityCommit = null;
  ...
  function getCert() {
    ...
    render(true); // garde la position (ne pas renvoyer en haut de page)
  ...
  function timeDetailText() {
    ...
    return 'Temps total : ' + fmtDur(totalTimeMs()) + '\n' + lines.join('\n');
  }
  ...
  function certDetailDoc() {
    ...
    return '<div id="certDocDetail" style="background:#fff;color:#111;border-radius:14px;padding:30px 28px;box-shadow:0 10px 30px rgba(0,0,0,.4);width:600px;box-sizing:border-box;font-family:\'Barlow\',sans-serif">'
      ...
      + '<div style="font-family:\'Barlow Condensed\',sans-serif;font-weight:800;letter-spacing:.06em;font-size:1.35rem;color:#d22325">ATTESTATION DE FORMATION</div>'
      + '<div style="color:#555;font-size:.9rem;margin:4px 0 6px">Prévention des troubles musculosquelettiques · Mine souterraine</div>'
      ...
      + '<div style="color:#888;font-size:.72rem">Suivi enregistré automatiquement · Formation guidée web</div></div></div>';
  }
  ...
  function bind() {
    ...
    // Zones du corps : pastille 3D OU bouton « zone » -> ouvre la fiche détaillée en surimpression (comme la base de connaissances).
