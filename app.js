...
// FR/EN flottant DESKTOP (coin bas-droit) : disparait au scroll, alterne avec .totop. Sur MOBILE (<=980) la bascule est dans la barre du bas (.botbar), toujours visible -> jamais masquee.
(function(){var ls=document.querySelector('.langswitch');if(!ls)return;var last=0,ticking=false;function upd(){var y=window.scrollY||document.documentElement.scrollTop||0;if(window.innerWidth<=980){ls.classList.remove('ls-hidden');last=y;ticking=false;return;}if(y<=140)ls.classList.remove('ls-hidden');else if(y>last+2)ls.classList.add('ls-hidden');else if(y<last-6)ls.classList.remove('ls-hidden');last=y<0?0:y;ticking=false;}upd();window.addEventListener('scroll',function(){if(!ticking){ticking=true;requestAnimationFrame(upd);}},{passive:true});})();
