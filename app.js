...
function calculerSim() {
  ...
  /* Miroir du résultat sur la page (cadran près du lien d'évaluation) */
...
(function () {
  ...
  var menuEls = document.querySelectorAll("[data-pwa-install]");          /* entree(s) « Installer l'app » du menu */
  ...
  function doInstall(){ if(deferred){ deferred.prompt(); deferred.userChoice.then(function(){ deferred=null; hide(); }); return; } if(standalone){ toast("&#10003; L'application est déjà installée sur cet appareil."); return; } toast(isIOS()?"Sur iPhone / iPad : appuie sur <b>Partager</b> puis <b>« Sur l'écran d'accueil »</b>.":"Pour installer : ouvre le menu de ton navigateur (&#8942;) puis <b>« Installer l'application »</b> ou <b>« Ajouter à l'écran d'accueil »</b>."); }
  if(box){ go.addEventListener("click", function(){ doInstall(); }); x.addEventListener("click", function(){ dismissed=true; hide(); }); }
  [].forEach.call(menuEls, function(el){ if(standalone){ el.style.display="none"; return; } el.addEventListener("click", function(ev){ ev.preventDefault(); doInstall(); }); }); /* deja installee -> on masque l'entree du menu */
  ...
  var hero=document.querySelector(".hero");                          /* le bandeau auto ne s'affiche que hors de la vue d'accueil */
